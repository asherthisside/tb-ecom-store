...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        shipping = ShippingAddress(customer=customer, order=cart, address=address, city=city, state=state, zipcode=zipcode)
        shipping.save()
    items = cart.orderitem_set.all()
    context = {'order': cart, 'items': items}
...
def quanplus(request, pk):
    customer = request.user.customer
    order = Order.objects.get(customer=customer, complete=False)
    item = order.orderitem_set.get(id=pk)
    item.quantity += 1
    item.save()
    return redirect("cart")

def quanminus(request, pk):
    customer = request.user.customer
    order = Order.objects.get(customer=customer, complete=False)
    item = order.orderitem_set.get(id=pk)
    item.quantity -= 1
